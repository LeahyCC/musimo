...
Library has Home, Albums, Artists, Tracks and Playlists views. One search field searches the active view; album, artist and track searches run against Navidrome, while the smaller playlist list is filtered in the browser. Sort choices are specific to each view. Album and track genre/year filters apply to the pages loaded so far, and more results load as you scroll. Grid and list layouts are available for collection views; list rows can open, play or shuffle albums and playlists. The Tracks list can play or shuffle all loaded results. Opening an album or playlist reveals its tracks and Play all creates one persistent queue. The footer player provides seek, previous, next, shuffle, repeat, volume and mute. The queue and position restore from Navidrome after a refresh. Browser media controls receive the current title, artist, album and artwork.
...
- `GET /api/player/lyrics/{id}`: structured lyrics.
