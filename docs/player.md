...
Space toggles library playback as well as previews. The Now Playing stage supports F for full screen, Esc to leave full screen or close the popout, left and right arrow keys to seek by five seconds, up and down arrows to adjust volume, M to mute, and N and P to skip tracks.

Persistence uses browser storage keys `musimo.player-volume`, `musimo.player-shuffle`, `musimo.player-repeat` and `musimo.library-layout`. The queue autosaves every 10 seconds of playback and on pause or close. Now playing scrobbles on play and submits on track end. Previous restarts the current track after 4 seconds of playback. Repeat one is available. MediaSession handlers provide play, pause, next and previous.
