...
A page-wide SQL query first recognizes completed catalog jobs whose saved path is still indexed, then matches ISRC, MBID, and normalized artist/title with duration within three seconds. Conflicting ISRCs cannot match through text. Higher-priority matches suppress lower-priority matches for the same result. The `Server-Timing` response header reports library lookup time separately from catalog and HTTP time.
