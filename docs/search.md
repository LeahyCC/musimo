...
The whole album or artist card is a native link. Artist links, preview, coverage retry and download controls remain separate targets. Card downloads sit over the top-right of the art, appear on hover or keyboard focus, and stay visible on touch screens. Card grids grow with the page instead of using a fixed-height inner scroller. Album card icons submit missing-only batches using saved settings and show queue feedback without navigation. Album pages have one download button using the selected format: Download album when no tracks are owned, Download missing (x) for a partial album, or a disabled Download album when verified coverage shows the entire album is in the library. Track rows reuse the shared download action, which is disabled for songs already in the library. URL imports remain unavailable. Original, M4A and Opus estimates assume 160 kbps; MP3 assumes 320 kbps. These are estimates, not promises about source quality.
