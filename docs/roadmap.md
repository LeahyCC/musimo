...
1. **Tailwind migration, themes and remaining layout polish.** Tailwind 4 is a dependency but does not yet style the app. [The styling plan](tailwind-migration.md) sequences tokens, a theme runtime with a personal settings page for choosing and building themes, shared primitives and screen-by-screen conversion, and lists the desktop/phone issues that should move with the screen that owns them (empty “View all”, Settings chrome before load, library error placement, Now Playing title overflow, breakpoint cleanup). Do not rewrite `style.css` in one change.
