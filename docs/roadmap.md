...
2. ✓ **Actionable source errors.** Link a job's error to the exact source/credential/disk setting that needs attention. Keep successful jobs visible when enrichment or Navidrome scanning warns. **Done:** each error code now carries a plain hint and a link to the relevant setting, diagnostic or action; warning counts appear in done job headings.
3. **Track navigation and context** (done 10 September 2026). Highlighted-track links now focus the row, show a visible focus indicator, and display a disabled state when no preview is available. Album and artist pages show "Back to results" when reached from search. Library playback already has previous, next, shuffle and repeat over a saved queue; the open question is whether previews should have previous and next, and if so, whether it follows the album, search results or an explicit preview queue.
...
6. ✓ **Long-list accessibility** (done 10 September 2026). Keyboard focus now works across virtualized lists without remounting, touch targets meet 44px minimum on coarse pointers, player height adapts dynamically to prevent overlap, automated axe checks run on every route, 9px text raised to 10px, and queue count changes announce via live region.
