...
4. ✓ **Explain duplicates and editions.** "Already queued", "in library" and "another edition matched" now show distinct badges and counts. Edition matches keep the download control enabled with a hint. Replacement flow remains open because the worker appends the job id to the filename when the target exists instead of overwriting.
