...
Choose a track format, then its download arrow. Songs already in the library have a disabled download arrow. Album cards download missing tracks. The album page has one button: Download album for an unowned album, Download missing (x) for a partial album, or a disabled Download album for a fully owned album. Track badges show queued work and failed downloads instead of calling both “Missing.” The bottom queue and Downloads page share live state. Pause stops the worker process; resume keeps usable partial files. Cancel removes that job’s staging folder. Retry restarts a failed or cancelled job. Queue controls, tabs and batch summaries show their failed counts, and the Failed tab groups the reasons before listing each job. When failures span more than one download group, the Failed tab offers a chip per group so an album’s failures can be read on their own; single-track failures group under Single tracks. Retry failed restarts every failure at once and Clear failed hides them; each finished card also has its own Clear. Clear all finished removes done, failed, and cancelled jobs from the queue. Clearing only hides a job, so History retains it and supports text and date filters. The History tab shows an empty state when nothing has finished yet.
