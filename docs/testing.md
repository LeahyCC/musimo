...
- Now Playing: `.stage`, `.stage-top`, `.stage-controls`, `.stage-overlay`, `img.stage-art`, `canvas.stage-visualizer`, `canvas.stage-hud`, and `.popout-body` in the popout. `.stage` and `.popout-body` also carry the fullscreen and popout rules, and the idle hook finds `.stage-overlay` too.
