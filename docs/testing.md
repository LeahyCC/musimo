...
- Search and album: `.track-row`, `.explicit`, `.album-actions`, `.download-target`, `.virtual-list` (also the handwritten list height rule).
...
- Now Playing: `.stage`, `.stage-top`, `.stage-controls`, `.stage-overlay`, `img.stage-art`, `canvas.stage-visualizer`, and `.popout-body` in the popout. `.stage` and `.popout-body` also carry the fullscreen and popout rules, and the idle hook finds `.stage-overlay` too.
