...
- Internet Archive, 19 September 2026 (HST), yt-dlp 2026.8.19, native downloader, no cookies: `scripts/source_probe.py --site-list --only archive` made **1 attempt and it succeeded in 3.307 s**. The URL was one MP3 track (about 1.9 MB, 69 s) of the Open Goldberg Variations, a CC0 release of Bach's Goldberg Variations. One attempt gives no median or p95, and the YouTube entry of the site list was not run because it needs the PO token provider. The link case of `scripts/download_smoke.py` then took the same item through resolve, queue, the real worker, tagging and indexing in **5.047 s**, and the resolver listed 32 entries for the 32 tracks.
- Indie sites, 19 September 2026 (HST), yt-dlp 2026.8.19, native downloader, no cookies: `scripts/source_probe.py --site-list --only <site>`, **one attempt each**, so no median or p95. Bandcamp succeeded in **7.447 s** (a 60 s track the artist offers as a free download, taken as FLAC), SoundCloud in **6.300 s** (160 kbps AAC), Audius in **4.282 s** (a 274 s track, 320 kbps MP3, 320,089 bps measured) and Jamendo in **20.521 s** (a 210 s FLAC of 460,687 bps). **Audiomack failed** in 1.598 s: yt-dlp's extractor stops with "Failed to parse JSON" on the live site. The URLs and why each is safe to use are in `scripts/source_probe_sites.json`; the YouTube entry was not run. The `bandcamp` case of `scripts/download_smoke.py` then took one track of a 60 track album through resolve, queue, the real worker, tagging and indexing in **15.203 s**, one attempt, giving a 60 s FLAC of 1,266,301 bps tagged with album, artist, album artist, date, track 3 of 60 and an embedded cover. See [more sources](more-sources.md#indie-sites-checked-for-real).
...
## Release continuation, 7 September 2026

The live database was inspected read-only. It held **58 NO_MATCH jobs across 45 distinct tracks**, and none retained review candidates. The worker now preserves up to three duration-valid rejected candidates when automatic matching fails, without selecting or downloading one. A focused regression test confirms the candidates remain available for manual listening and selection.

A reproducible corpus now contains **100 distinct MusicBrainz recordings** paired with public YouTube references through unique Wikidata recording and video identifiers. Live `ytsearch8` results supply the saved candidates. The corpus includes 63 ambiguous-title cases, 13 live decoys, 9 instrumental decoys, 6 cover decoys, 6 remix decoys, 3 karaoke decoys and 7 explicit version targets. The linked reference appeared in search for 69 cases, ranked first for 28 and ranked in the accepted top three for 36. These are retrieval diagnostics, not precision. Independent listening labels are pending, so accuracy is **NOT MEASURED**. See [review method](match-review.md), [corpus](evidence/match-review-corpus.json) and [benchmark result](evidence/match-benchmark.json).

SoundCloud match precision is **NOT MEASURED**. `scripts/match_benchmark.py --source soundcloud` now exists and reads a case's `soundcloud_candidates` and `ground_truth.soundcloud_ids`, but the 100-case corpus holds YouTube rows only, so there is nothing to score yet. The SoundCloud thresholds (0.70 to accept, 0.90 for "check match") are chosen, not measured, and the setting that uses them is off by default until they are.

Ten distinct cold catalog searches against an isolated empty test library had median **416.95 ms** and p95 **496.56 ms**. The provider dominated at median **410.30 ms** and p95 **479.55 ms**. Queue, catalog parsing and library lookup each stayed below 1.2 ms p95. Fifty cached requests had p95 **3.05 ms**. In Chromium, debounce-to-request p95 was **228.1 ms**, response-to-paint p95 was **61.1 ms**, and total input-to-paint p95 was **792.0 ms**. The server's 400 ms cold target remains unmet, but no local queue, parsing, index or cache bottleneck was demonstrated. See [HTTP timing](evidence/release-search-timing.json) and [browser timing](evidence/release-search-browser.json).

The production image built for `linux/arm64` under QEMU, booted as `aarch64`, became healthy and passed all **41 backend tests** with Python 3.14.7. FFmpeg and native AArch64 Deno also ran. Native ARM hardware and performance remain **NOT MEASURED**. See [ARM64 evidence](evidence/arm64-emulated.json).
