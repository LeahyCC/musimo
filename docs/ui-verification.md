...
`e2e/player.spec.ts` covers the loading transition check. `e2e/now-playing-popout.spec.ts` checks the Now Playing hover and idle stage on desktop Chromium only, and the stage-first layout: the stage's share of the width, the details beside it, the footer hidden with `--player-height` at `0px`, the picker still opening, a short window keeping the stage's controls in view, and the phone's full-width stage with no mini player. `e2e/phone.spec.ts` checks the mini player steps aside once Now Playing opens from it, and `e2e/themed-walk.spec.ts` expects no footer on that route.
