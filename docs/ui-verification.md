...
`e2e/player.spec.ts` covers the loading transition check. `e2e/now-playing-popout.spec.ts` checks the Now Playing hover and idle stage on desktop Chromium only.
