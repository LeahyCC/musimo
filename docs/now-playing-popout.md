...
L and Q are bound to the page, not the stage, so they work without the stage focused.
...
- `frontend/src/now-playing-shortcuts.tsx`: the page's keys (`usePageShortcuts`, and `pageKeyIsFree`, the rule for when a key may be taken, which the tabs' L and Q use too) and the cheat sheet dialog.
- `frontend/src/now-playing-wash.tsx`, `frontend/src/cover-color.ts`, `frontend/src/wash.ts`: the backdrop. `NowPlayingWash` draws the wash and `NowPlayingGlow` the glow (which `NowPlayingStage` places in the stage's slot), `cover-color.ts` reads the cover, and `wash.ts` holds the rules: `washFor`, which keeps the text readable, and `glowFor`, the fixed mix (unit tested in `wash.test.ts`, with `cover-color.test.ts` for the colour pick and `theme/color.test.ts` for the mix). `frontend/src/use-leaving.ts` is the 300 ms cross-fade, shared by the wash, the glow and the artwork.
