...
The shell had two rules in that band. The save bar's only extra rule was the 767 px padding, which is now `max-phone:p-[12px]`. The queue dock's was `right: calc(12px + var(--safe-right))` at 700 px, and it was already unreachable: the dock is hidden under 768 px, so no window narrow enough to want the smaller offset ever draws it. That rule is deleted rather than folded. The 700 px rules still in the sheet belong to the queue sheet and job cards (phase 6) and the Now Playing hero and stage controls (phase 8); each of those screens makes the same call when it converts.
