# Tailwind migration and styling plan

Tailwind 4 is already a dependency (`tailwindcss` 4.3.3, `@tailwindcss/vite`, `@import 'tailwindcss'` in `frontend/src/style.css`). It does not style the app yet. Every screen is painted by about 3,900 lines of handwritten CSS and 223 semantic class names. This note is the plan for finishing that move, and for fixing the desktop and phone layout work that the current sheet makes expensive.

It is not a visual redesign. Dark theme only stays. Native dialogs, range inputs, meters and the Now Playing popout stay. shadcn / Base UI stay out; [architecture](architecture.md) already chose a smaller set.
...
Migrating to Tailwind is the way to make the next desktop and phone fixes local: a component owns its classes, a token owns a color, a named breakpoint owns a layout change.
...
The popout copies every live stylesheet into the new document (`copyStyles` in `now-playing-popout.tsx`). Generated Tailwind CSS is copied the same way as today’s sheet. Do not split critical popout/stage rules into a document the opener cannot read.
...
| `Panel` / `EmptyPanel`             | `.panel`, `.empty-panel`, `.library-empty`     | —                                     |
| `ErrorBanner` / `InlineError`      | `.error`, `.inline-error`, `.failure-summary`  | —                                     |
...
Work in stacked pull requests. Each PR is one surface, keeps the app usable, and is verified on 1280×800 and 390×844 (plus 360×780 for phone chrome). Do not convert the whole sheet in one change.

```text
0  This plan
1  Tokens, variants, --text, cx helper          no intended visual change
2  Primitives (button, icon, chip, error, field)
3  Shell + footer player + save bar + dock      desktop / phone chrome
4  Search, cards, track rows, album / artist
5  Library grids, lists, toolbars, detail
6  Downloads, job cards, queue sheet, options
7  Settings, diagnostics, activity
8  Now Playing stage, overlay, popout leftovers
9  Delete dead CSS; retarget leftover class locators
```
...
- Leave class names and markup alone. Diff should be CSS-only plus a one-line architecture note.
...
7. Update [UI verification](ui-verification.md) only when a walk finds or closes a user-visible issue. Do not log a class-only conversion there.

## Out of scope

- Light theme, theming runtime, user-selectable accent.
...
Phase 1 only: tokens, `--text`, custom variants, breakpoint names, and a short “Styling” subsection in [architecture](architecture.md) that points here. No class renames. That gives every later UI/UX change a palette and a `max-phone:` / `coarse:` language without risking the phone chrome.
