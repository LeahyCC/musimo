...
**Tokens and themes.** Colors are `--color-*` tokens in `@theme`, named for their job (`canvas`, `raised`, `muted`, `danger-bg`), never for a shade. Tailwind's stock palette is switched off, so `bg-raised` works and `bg-red-500` does not exist. A theme is one hex value per token. The default needs no JavaScript because `@theme` already holds it. Any other theme is written onto `<html>` (and onto the Now Playing popout while it is open) with `setProperty`, and the utilities repaint. `frontend/src/theme/` holds the token list, the built-in registry, the zod schema for stored and imported themes, the contrast helpers and the store. A built-in theme may also carry a skin, a sheet of shape rules keyed on `<html data-skin>`; Windows 95 is the only one, in `frontend/src/theme/win95.css` (see [Skins](tailwind-migration.md#themes)). `frontend/public/theme-boot.js` paints a saved theme before the first frame, so there is no flash of the default. It is a classic external script because the Content-Security-Policy allows no inline script.

**The no-raw-color rule.** No hex, `rgb()`, `hsl()` or named color anywhere outside the `@theme` block, and no arbitrary color utility such as `bg-[#17201b]`. A translucent color is an opacity modifier (`bg-accent/20`) or `color-mix(in oklab, var(--color-x) N%, transparent)`, and that includes shadows, washes and backdrops. `frontend/src/no-raw-colors.test.ts` reads every sheet (the skin's too) and every component and fails with the file and line. The only file it excuses is the theme registry.
...
**Adding a built-in theme.** Add one object to `BUILT_IN_THEMES` in `frontend/src/theme/themes.ts` with an id, name, scheme and a value for every token. `contrast.test.ts` fails it on any pair in the editor's readability report under WCAG AA, then walk the app under it the way `e2e/themed-walk.spec.ts` walks the light fixture. Leave `musimo-dark` equal to the `@theme` block: `themes.test.ts` compares them.
...
- `POST /api/links/resolve`: checks the link against the site allowlist, reads it in a child process within 20 seconds without downloading (at most 500 entries), and returns a preview with a token that lasts ten minutes. The preview names the site, says whether the link is a single recording or a profile, and carries the site's `quality_note` and a `partial` flag. Each entry has its `kind` and, for a mix or radio show, the `lands` path it will be filed under. A refusal is 422 with a plain sentence, a lookup that runs out of time is 504, and a browser that cancels gets 499. Two lookups run at once and the server keeps 32 previews.
- `POST /api/links`: queues ticked entries from a saved preview as `link` jobs; more than one becomes a download group. An expired token is 404 and an entry that is not in the preview is 422.
...
- `GET /api/player/waveform/{id}` (waveform.py): about 800 normalized peaks for the Now Playing seek bar. One ffmpeg decode from the file on disk when Navidrome's path resolves inside a library root, otherwise from a copy of the stream; cached in `waveforms`; one decode at a time with 8 waiting, 404 when there are none or a wait or decode times out, 503 when the line is full.
...
- `error_guidance(code, site)`: 19 code hint and fix table used by worker.py, downloads.py and store.py job_summary. Maps error codes to plain language hints and suggested actions, naming the job's site where the hint is about one.
- `source_code(code, source)`: keeps a code only where it means something for the job's source.

Not yet implemented:

- Catalog imports from Spotify or Apple Music links. A pasted one is refused with "Catalog imports are not built yet."
- AcoustID identification of a pasted link whose metadata is junk.
- Audiomack and TuneIn links. Their rows are in the site table with `working` off, because yt-dlp cannot read either site today.
- Cookie uploads and source test UI.
- Notification test endpoints.
- Updater controls.
- Lyric backfill jobs.

Same-origin JSON writes reject cross-origin browser requests. No wildcard CORS. The service binds loopback and accepts no secrets. Optional password sessions, secret files and encrypted credential storage precede LAN-facing credential features. Arbitrary URLs, output paths, redirects and yt-dlp arguments require allowlists before download APIs ship; deny executable hooks and output overrides. Pasted links meet this through `backend/sources.py`: only `https` links to a listed host are read, never an IP address host, a link with a user or password, or a non-default port. yt-dlp gets the site's extractors as `allowed_extractors`, which turns off the generic extractor, so a redirect to an unlisted site fails in both the preview and the worker. The worker also checks that a job's own address belongs to the site the job says it is from, and a job whose site has left the table fails with `SITE_NOT_ALLOWED`. Artwork is fetched by the server, so its hosts are on the same list and a redirect is followed only to another of that site's image hosts. The browser sends only the link and then a preview token with entry IDs; titles, file addresses, artwork and output paths come from the server's saved preview, and no cookies or yt-dlp options are accepted.
...
Podcast episodes and pasted links skip `matching` and go straight to `downloading`. A catalog track is matched on YouTube, and on SoundCloud as well when `soundcloud_fallback` is on, after YouTube found nothing or while YouTube is paused; a match there changes the job's `source` to `soundcloud`. Conversion is skipped when possible; remuxing is recorded distinctly from lossy encoding. A pause during tagging either finishes that artifact or restarts tagging later, never continues a corrupt partial tag write. Pause intent remains durable through process exit and container restart. A restart reconciles active stages against the manifest before dispatch. Errors retain stage, code, retryability, a redacted tool tail and tool version. Three consecutive blocking errors (`SOURCE_BLOCKED`, `POT_MISSING`, `JS_RUNTIME_MISSING`, `COOKIES_EXPIRED`) pause one source, not unrelated healthy sources: each job records its `source`, the count and pause are kept per source in `source_control`, a completed download from that source resets its count, and the dispatcher skips a queued job only when its own source is paused. A catalog track whose own source is paused but which may fall back to SoundCloud is dispatched there instead of waiting. `GEO_RESTRICTED` and a podcast host's refusals never count toward a pause. Disk-full/unwritable jobs require a successful destination probe before retry.
...
- **Now Playing:** Artwork stage with full screen and popout, the track's controls under it, and one tabbed panel of Up next, Lyrics, About and History beside it (beneath on a phone). Library tracks only. Its seek bar draws the track's waveform and falls back to the plain bar (see [the player note](player.md#waveform-seek-bar)). See [the popout note](now-playing-popout.md#layout).
