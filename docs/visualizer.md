...
Each frame: read the analyser through the feature client (once the library element has played), stamp time and dt into the packet, upload it as one 64-byte uniform, resolve the preset's audio mapping into the scene's numbers and the post stack's, run the scene's compute and render passes into the post stack's texture, run the stack over it onto the canvas, then draw the HUD if it is on. React owns mounting, unmounting and the controls only; no per-frame state touches it. The canvas carries `data-adapter`, `data-frame-ms`, `data-scene`, `data-detail` (the scene's workload, such as `250,000 particles`, `512 fluid` or `64-step raymarch`), `data-post` and `data-preset` so a screenshot or a test can read them.
...
Three of them, behind the `Scene` interface in `scenes/Scene.ts`: `init`, `resize`, `update`, `render`, `dispose`, and a `detail` line naming what the scene is doing. The renderer holds exactly one, named by the chosen preset and remembered as `musimo.visualizer-scene`; the default is the particle field.
...
Sixteen numbers in `scenes/particles.params.ts` say how hard the flow pushes, how far the attractors reach, how fast a dead particle comes back and how bright a particle comes out. They used to be constants in the compute shader with the packet multiplied into them on the GPU; they are on the CPU now and arrive already modulated, which is why the shader reads the packet for nothing but the clock. The default preset drives them exactly as PR #56 tuned them: bass to attractor strength, treble to noise frequency and jitter, `beatPulse` to a radial push and a size bump, energy to flow strength, speed and respawn, and the colour cool to warm with bass.

The two pipelines have explicit bind group layouts rather than `layout: 'auto'`. A derived layout only holds the bindings a shader happens to read, so when the render half stopped reading the packet it lost the feature binding and every bind group built for both halves became invalid; naming the layout means the two agree whatever the shaders read.
...
Twelve numbers in `scenes/raymarch.params.ts` are what a preset moves: the fold scale and how many times it runs, where the camera orbits and how far in something may pull it, the light, the halo, the occlusion and the colour ramp. The camera never comes nearer than `closest`, whatever drives `pullIn`, because inside the fold the distance estimate is no use and the frame turns to noise. The iteration count is rounded and capped, so nothing a preset asks for can make one frame cost ten times the last.

The default preset keeps PR #59's tuning: bass sets the fold scale, treble sets how many times it runs, `beatPulse` pulls the camera in, and energy drives the light and the halo. The other preset for this scene swaps the first two round, which is the whole point of the mapping table being a table.
...
All of it is resolved on the CPU, once a frame, in `presets/resolve.ts`, and nowhere else. That is why a preset can send treble to a fold that used to take bass without a line of WGSL changing, and why the scenes read no magnitude from the packet at all. Both resolvers write into an object the renderer owns and keeps, since this runs on every animation frame.

The six, two per scene, in the order `[` and `]` walk them:

| Preset  | Scene     | What it does differently                                                                                                                                                                                    |
| ------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Drift   | Particles | The field as PR #56 tuned it, and the default. Bass to the attractors, treble to the noise and the jitter, energy to the flow.                                                                              |
| Storm   | Particles | Treble to the attractors and bass to the jitter, so the cloud is drawn in by the hats rather than the kick. Faster orbit, harder beat push, treble on the feedback and energy on the bloom.                 |
| Plume   | Fluid     | The fluid as PR #58 tuned it. Bass decides what an onset is worth, treble the vorticity.                                                                                                                    |
...
| Fold    | Raymarch  | The march as PR #59 tuned it. Bass to the fold scale, treble to the iterations.                                                                                                                             |
| Furnace | Raymarch  | Treble to the fold scale and bass to the iterations, the swap the card asked for. Camera further back and turning faster, a hotter ramp, energy on the bloom threshold and the chromatic split.             |

`postParams` is where a preset changes the look without touching the scene: Storm and Furnace both raise the bloom and open the chromatic split, and Wash lifts the feedback and the threshold so its thinner plumes keep their edges.
...
presets/broken.json: audioMapping[0].to is neither a knob of this scene (flow, …) nor a post target (feedback.amount, …)
```
...
The scene select and the size select beside it still work. A preset names a scene, so choosing one moves the scene select under it, and choosing a scene moves to that scene's first preset; the two are never left disagreeing. That makes the scene select redundant, since every scene has a preset and the picker reaches all six, but the size control is not, so the select stays for now rather than being pulled out in the same step.
...
- `frontend/src/visualizer/presets/`: the six preset files, the knob vocabulary (`knobs.ts`), the parser (`parse.ts`), the mapping resolved once a frame (`resolve.ts`) and the list the top bar reads (`index.ts`).
- `frontend/src/visualizer/scenes/Particles.ts`, `scenes/particles.params.ts` and `shaders/particles.*.wgsl`: the particle scene, its numbers and its two passes.
...
- `frontend/src/now-playing-overlay.tsx` and `now-playing-popout.tsx`: the preset picker, the `[` and `]` keys, and the state that keeps the preset and the scene select in step.
...
- Presets, on the same Mac in headed Chromium 153 against the real library, two of the tracks above: Aphex Twin's Donkey Rhubarb and CamelPhat's Easier in the Sub Focus remix. Each was searched for, started from the tracks list and opened with in-app navigation so the analyser survived, with the artist on the footer checked before anything was read. All six presets were cycled on each track at both sizes, `data-frame-ms` read after ten and a half seconds, and the stage screenshotted at 3840 by 2160. No console errors and no `uncapturederror` across the run. The display runs at 120 Hz, so 8.3 ms means the GPU is not the limit:

  | Stage size, track                                      | Drift  | Storm  | Plume  | Wash   | Fold    | Furnace |
  | ------------------------------------------------------ | ------ | ------ | ------ | ------ | ------- | ------- |
  | 320 by 320, ratio 1 (docked), Donkey Rhubarb           | 8.3 ms | 8.4 ms | 8.3 ms | 8.3 ms | 8.3 ms  | 8.3 ms  |
  | 320 by 320, CamelPhat                                  | 8.3 ms | 8.4 ms | 8.2 ms | 8.4 ms | 8.3 ms  | 8.2 ms  |
  | 1920 by 1080 at ratio 2 (3840 by 2160), Donkey Rhubarb | 8.4 ms | 8.3 ms | 8.3 ms | 8.4 ms | 19.1 ms | 15.4 ms |
  | 3840 by 2160, CamelPhat                                | 8.3 ms | 8.3 ms | 8.4 ms | 8.3 ms | 20.0 ms | 21.6 ms |

  The mapping costs nothing measurable: a preset resolves a dozen numbers a frame on the CPU. The only cells that leave the cap are the two raymarch presets at 4K, which is that scene's own cost and was already the case before presets. Furnace is not reliably the cheaper of the two: it sits further back so the fold covers less of the frame, but bass drives its iteration count and a bass-heavy passage folds more often, which is why it reads faster than Fold on one track and slower on the other. Full screen cannot be requested without a gesture under automation, so the 4K case forces the stage to a 1920 by 1080 CSS box at ratio 2, which is the same canvas.

- What each preset showed, screenshotted at 3840 by 2160. Drift is the field as it was, checked against the same moment of the same track built from `main`: a blue cloud filling the frame with bright cores, the same either way. Storm draws that cloud in around one hot core, because the attractors follow the hats rather than the kick. Plume gives the interlocking orange, magenta and teal masses PR #58 tuned. Wash gives three separate slow plumes with visible emitter heads and heavy fringing along their edges on a mostly black frame. Fold is the folded box seen close, in gold and teal, with the faces broken into filigree down to a pixel. Furnace pulls the camera back so the whole shape is in the frame, blue and violet against a hot orange halo, the fold running smoother because treble rather than bass is opening it.

- Two of the six were retuned after looking at them. Wash was first written with the emitters at 0.62 of the visible band and a faster dye decay, which is exactly the look PR #58 rejected as a default: three plumes with black between them and over half the frame empty. Pulling the spread back to 0.5 and raising the trickle keeps it airier than Plume without being empty. Furnace was first written with the camera at 6.4 and a floor of 5.4, which put it against the fold: the frame was one wall of detail, and it looked the same on both tracks because nothing about the shape was visible to change. At 7.6 with a floor of 6.2 the whole box is in frame and the beat pulling it in reads.

- The picker, `[` and `]`, and the remembered choice, in headed Chromium against the real library: `]` walks all six in order and wraps, `[` walks back and wraps, and `data-preset`, `data-scene`, `data-detail`, the picker and the scene select follow every step. The choice survives a reload as `musimo.visualizer-preset`, and choosing a scene from the select moves the preset to that scene's first, so the two controls are never left disagreeing. H names the preset on the overlay. No console errors.
...
- One bug was found this way and not by reading. The particle pipelines were built with `layout: 'auto'`, whose derived layout holds only the bindings a shader happens to read. Taking the feature packet out of the render shader dropped binding 0 from that layout, and the bind group built for both halves became invalid: every particle frame failed validation and the two particle presets drew nothing, with thousands of WebGPU errors in the console. Both pipelines have explicit layouts now.
