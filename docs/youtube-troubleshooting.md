...
Keep yt-dlp's default client selection. The image pins yt-dlp and bgutil; update through a rebuilt, tested image for now. The plugin and helper are both pinned to bgutil 2.0.0, which fixes the HTTP server issues described in its [release notes](https://github.com/Brainicism/bgutil-ytdlp-pot-provider/releases/tag/2.0.0). Update both together, including native helper checkouts. The Docker helper remains accessible only within its container network, with no published port. A safe hot updater with separate installations, self-test and rollback is not implemented yet.
