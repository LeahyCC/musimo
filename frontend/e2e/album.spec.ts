...
test('album download button is disabled while coverage is unverified', async ({ page }) => {
  await page.route('**/*', async (route) => {
    if (new URL(route.request().url()).origin !== ORIGIN) await route.abort()
    else await route.fallback()
  })
  ...
  const tracksUnverified: MusicResult[] = [0, 1].map((index) => ({
    ...track,
    id: 101 + index,
    title: `Recording ${index + 1}`,
  ...
  await expect(action).toHaveText('Checking coverage')
})
