...
export type StudyChoice = { id: string; name: string; disabled: boolean }
...
export function useOverlayIdle(container: RefObject<HTMLDivElement | null>, playing: boolean) {
  ...
  useEffect(() => {
    ...
    const rest = () => {
      if (!playing) return
      if (element.querySelector('.visual-overlay :hover')) {
  ...
  return (
    <div className="visual-overlay">
      <div className="visual-top">
        <select
          aria-label="Visual study"
          value={study}
          onChange={(event) => onStudy(event.target.value)}
        >
          {choices.map((choice) => (
        ...
        <div className="visual-actions">
          {onPopout && (
            <button className="icon-button" aria-label="Pop out visualizer" onClick={onPopout}>
