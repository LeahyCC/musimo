...
class SearchTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_search_deadline_includes_rate_wait_and_album_coverage_is_exact(self) -> None:
        ...
        async with httpx.AsyncClient(transport=httpx.MockTransport(transport)) as upstream:
            ...
            async with httpx.AsyncClient(
                ...
                top = (await client.get("/api/artists/27/top")).json()
                self.assertEqual([track["id"] for track in top["tracks"]], [1])
                self.assertEqual((await client.get("/api/artists/0/top")).status_code, 422)
    ...
    async def test_preview_stream_validates_redirects_type_and_size(self) -> None:
        for variant in ("audio", "redirect", "html", "oversize"):
            with self.subTest(variant=variant):
                requested: list[str] = []

                def upstream(
                    request: httpx.Request, variant: str = variant, requested: list[str] = requested
                ) -> httpx.Response:
                    ...
                    if variant == "html":
                        return httpx.Response(
                            200, text="wrong content", headers={"content-type": "text/html"}
                        )
                    return httpx.Response(
                        200,
                        content=b"a" * (10 * 1024 * 1024 + 1)
                        if variant == "oversize"
                        else b"audio fixture",
                        headers={"content-type": "audio/mpeg"},
                    )

                media_client = httpx.AsyncClient(transport=httpx.MockTransport(upstream))
                async with httpx.AsyncClient(
                    transport=httpx.MockTransport(lambda _: httpx.Response(200, json=TRACK))
                ) as catalog_client:
                    catalog = Catalog(self.store, catalog_client)
                    app = FastAPI()

                    def get_catalog(catalog: Catalog = catalog) -> Catalog:
                        return catalog

                    install_search_routes(app, get_catalog, lambda: self.library)
                    async with httpx.AsyncClient(
                        transport=httpx.ASGITransport(app), base_url="http://test"
                    ) as client:
                        with patch(
                            "backend.search_api.httpx.AsyncClient", return_value=media_client
                        ):
                            response = await client.get("/api/preview/1/stream")
                        self.assertEqual(response.status_code, 200 if variant == "audio" else 502)
                        if variant == "audio":
                            self.assertEqual(response.content, b"audio fixture")
                self.assertEqual(requested, ["cdnt-preview.dzcdn.net"])
